...
public class InitializationBlocksHandlerTest {
  ...
  public void testInitializationBlocksInThreeVersions() {
    ...
    assertTrue(
        mergeResult.contains("<<<<<<<MINE_name=\"Left\";=======_name=\"Right\";>>>>>>>YOURS"));
  ...
  public void testInitializationBlocksDistincts() {
    ...
    assertTrue(
        mergeResult.contains(
            "<<<<<<<MINESystem.out.println(\"Left\");=======try{Class.forName(\"Right\");}catch(ClassNotFoundExceptione){e.printStackTrace();}>>>>>>>YOURS"));
    assertTrue(ctx.initializationBlocksConflicts == 1);
  }
}
