...
/**
 * Class responsible for converting ASTs into source code and responsible for operations related to
 * printing/generating merged code.
 *
 * @author Guilherme
 */
public final class Prettyprinter {
  ...
  public static String print(FSTNode tree) {
    ...
    String printable = "";
    // de.ovgu.cide.fstgen.parsers.generated_java18_merge.SimplePrintVisitor printer = new
    // de.ovgu.cide.fstgen.parsers.generated_java18_merge.SimplePrintVisitor();
    ...
    // printable = printable.trim().replaceAll(" +", " "); //fix for a bug in
    // java18_merge.SimplePrintVisitor, not working
    return printable;
  }
  ...
  public static void printOnScreenMergedCode(MergeContext context) {
    ...
    System.out.println(
        (context.semistructuredOutput.isEmpty()
            ? "empty (deleted, inexistent or invalid merged files)\n"
  ...
  /**
   * Prints the merged code result of both unstructured and semistructured merge in the given output
   * file.
   *
   * @param context
  ...
  public static void generateMergedFile(MergeContext context, String outputFilePath)
    ...
    if (outputFilePath != null) {
      if (outputFilePath.isEmpty())
        outputFilePath =
            context.getRight().getAbsolutePath(); // merging mine(left) into yours(right)
      String semistructuredOutputFilePath = outputFilePath;
      String semistructuredMergeOutputContent = context.semistructuredOutput;
      boolean writeSucceed =
          FilesManager.writeContent(semistructuredOutputFilePath, semistructuredMergeOutputContent);
      if (writeSucceed && !JFSTMerge.isGit) {
        ...
        writeSucceed =
            FilesManager.writeContent(unstructuredOutputFilePath, unstructuredMergeOutputContent);
      }
      if (!writeSucceed) {
  ...
  public static void generateMergedScenario(MergeScenario scenario) throws PrintException {
    String mergedRevisionId =
        "rev_" + scenario.getLeftRevisionID() + "-" + scenario.getRightRevisionID();
    List<FilesTuple> tuples = scenario.getTuples();
    for (FilesTuple mergedTuple : tuples) {
      String mergedDirectory =
          mergedTuple.getOutputpath().replace(scenario.getRightRevisionID(), mergedRevisionId);
