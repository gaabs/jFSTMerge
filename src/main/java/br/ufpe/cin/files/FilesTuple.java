...
/**
 * Represents a triple of matched files. That is, files with the same name from the same directory
 * in the three revisions being merged (three-way merge). It also stores the output of both
 * unstructured and semistructured merge.
 *
 * @author Guilherme
 */
public class FilesTuple {
  ...
  public String toString() {
    return "LEFT: "
        + ((leftFile == null) ? "empty" : leftFile.getAbsolutePath())
        + "\n"
        + "BASE: "
        + ((baseFile == null) ? "empty" : baseFile.getAbsolutePath())
        + "\n"
        + "RIGHT: "
        + ((rightFile == null) ? "empty" : rightFile.getAbsolutePath());
  }
  ...
  public boolean equals(Object obj) {
    if (obj instanceof FilesTuple) {
      ...
      return thisleftid.equals(otherleftid)
          && thisbaseid.equals(otherbaseid)
          && thisrightid.equals(otherrightid);
