...
public final class FilesManager {
  ...
  public static List<FilesTuple> fillFilesTuples(String leftDir, String baseDir, String rightDir) {
    ...
    // searches corresponding files beginning from files in the base version, followed by files in
    // left version, and finally in files in right version
    searchCorrespondingFiles(
        leftDir,
        baseDir,
        rightDir,
        tuples,
   ...
   * @return list of tuples of matched files
   */
  public static List<FilesTuple> fillFilesTuples(
      String leftDir,
      String baseDir,
      String rightDir,
      String outputpath,
    ...
    for (String l : filesFromLeft) {
      ...
      FilesTuple tp =
          new FilesTuple(
              ...
              baseFile,
              rightFile,
              ((null != outputpath && !outputpath.isEmpty()) ? outputpath : rightDir));
    ...
    for (String sl : subdirectoriesFromLeft) {
      ...
      if (!visitedPaths.contains(
          sl
              + (baseDir + File.separator + foldername)
              + (rightDir + File.separator + foldername))) {
        List<FilesTuple> tps =
            fillFilesTuples(
                sl,
                (baseDir + File.separator + foldername),
    ...
    for (String sb : subdirectoriesFromBase) {
      ...
      if (!visitedPaths.contains(
          (leftDir + File.separator + foldername)
              + sb
              + (rightDir + File.separator + foldername))) {
        List<FilesTuple> tps =
            fillFilesTuples(
    ...
    for (String sr : subdirectoriesFromRight) {
      ...
      if (!visitedPaths.contains(
          (leftDir + File.separator + foldername) + (baseDir + File.separator + foldername) + sr)) {
        List<FilesTuple> tps =
            fillFilesTuples(
                (leftDir + File.separator + foldername),
                (baseDir + File.separator + foldername),
  ...
  /**
   * Given a main list of files path, searches for corresponding files in other two given files path
   * list.
   *
   ...
   */
  private static void searchCorrespondingFiles(
      String firstVariantDir,
      String mainDir,
      String secondVariantDir,
    ...
    while (!filesPathFromMainVariant.isEmpty()) {
      ...
      String correspondingFirstVariantFilePath =
          replaceFilePath(baseFilePath, mainDir, firstVariantDir);
      String correspondingSecondVariantFilePath =
          replaceFilePath(baseFilePath, mainDir, secondVariantDir);
  ...
  /**
   * Validate the given files by verifying if they exist. In case of non-existing file, the
  ...
  public static List<MergeConflict> extractMergeConflicts(String mergedCode) {
    ...
    try (BufferedReader reader = new BufferedReader(new StringReader(mergedCode))) {
      ...
      while (itlines.hasNext()) {
        ...
        if (line.contains(CONFLICT_HEADER_BEGIN)) {
          ...
          MergeConflict mergeConflict =
              new MergeConflict(leftConflictingContent, rightConflictingContent, startLOC, endLOC);
  ...
  /**
   * Finds a node with the content in the first parameter, and replace the content with the content
   * in the second parameter.
   *
   * @param tree
   * @param newContent
   */
  public static boolean findAndReplaceASTNodeContent(
      FSTNode node, String oldContent, String newContent) {
  ...
  /**
   * Finds a node with the given content and deletes it from the given tree.
   *
  ...
  public static String estimateProjectRootFolderPath(MergeContext context) {
    ...
    if (left != null && left.getAbsolutePath().contains(File.separator + "src" + File.separator)) {
      ...
      rootFolderPath = (left.getAbsolutePath().substring(0, srcidx)) + File.separator;
        ...
        && base.getAbsolutePath().contains(File.separator + "src" + File.separator)) {
      ...
      rootFolderPath = (base.getAbsolutePath().substring(0, srcidx)) + File.separator;
        ...
        && right.getAbsolutePath().contains(File.separator + "src" + File.separator)) {
  ...
  public static String indentCode(String sourceCode) {
    ...
    try {
      CompilationUnit indenter =
          JavaParser.parse(
              new ByteArrayInputStream(sourceCode.getBytes()),
              StandardCharsets.UTF_8.displayName());
      indentedCode = indenter.toString();
    } catch (Exception e) {
    } // in case of any errors, returns the non-indented sourceCode
    return indentedCode;
  }
   ...
   */
  public static boolean areFilesDifferent(
      File left, File base, File right, String outputFilePath, MergeContext context) {
    boolean result = true;

    // reading files content
    String auxleft = FilesManager.readFileContent(left);
    String leftcontent = (auxleft == null) ? "" : auxleft;
    String leftcontenttrim =
        (auxleft == null) ? "" : FilesManager.getStringContentIntoSingleLineNoSpacing(auxleft);

    String auxright = FilesManager.readFileContent(right);
    String rightcontent = (auxright == null) ? "" : auxright;
    String rightcontenttrim =
        (auxright == null) ? "" : FilesManager.getStringContentIntoSingleLineNoSpacing(auxright);

    String auxbase = FilesManager.readFileContent(base);
    String basecontentrim =
        (auxbase == null) ? "" : FilesManager.getStringContentIntoSingleLineNoSpacing(auxbase);
