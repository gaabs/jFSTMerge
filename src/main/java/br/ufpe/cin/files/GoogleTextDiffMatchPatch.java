 ...
 *
 * <p>Computes the difference between two texts to create a patch. Applies the patch onto another
 * text, allowing for errors.
 *
 * <p>This code has been adapted from <a
 * href="http://code.google.com/p/google-diff-match-patch">google-diff-match-patch</a> library by
 * <b>Neil Fraser</b> (fraser@google.com).
 *
 * <p>I've changed the code to embrace java encapsulation, naming conventions, and added a
 * "word-level" text diff feature.
 *
 * @author Tine Kondo
 */
public class GoogleTextDiffMatchPatch {
  ...
  private static final String TAG_REGEX =
      "<(\"[^\"]*\"|'[^']*'|[^'\">])*>"; // in case the texts being compared contain XML or HTML
                                         // tags
  ...
  protected static class Patch {
    ...
    public String toString() {
      ...
      else if (this.length1 == 1) coords1 = Integer.toString(this.start1 + 1);
      else coords1 = this.start1 + 1 + "," + this.length1;
      if (this.length2 == 0) coords2 = this.start2 + ",0";
      else if (this.length2 == 1) coords2 = Integer.toString(this.start2 + 1);
  ...
  /**
   * Internal class for returning results from diffLinesToChars(). Other less paranoid languages
  ...
  protected static class LinesOrWordsToCharsResult {
    ...
    protected LinesOrWordsToCharsResult(
        final String chars1, final String chars2, final List<String> lineArray) {
      this.chars1 = chars1;
      this.chars2 = chars2;
   ...
   *
   * <p>{@code Diff(Operation.DELETE, "Hello"), Diff(Operation.INSERT, "Goodbye"),
   * Diff(Operation.EQUAL, " world.")} which means: delete "Hello", add "Goodbye" and keep " world."
   */
   ...
   * allows the 'checklines' of diffMain() to be optional. Most of the time checklines is wanted, so
   * default to true.
   *
   * @param text1 Old string to be diffed.
   * @param text2 New string to be diffed.
  ...
  public List<Diff> diffMain(final String text1, final String text2, final boolean checklines) {
    ...
    long deadline;
    if (diffTimeout <= 0) deadline = Long.MAX_VALUE;
    else deadline = System.currentTimeMillis() + (long) (diffTimeout * 1000);
   ...
   * @return Linked List of Diff objects.
   */
  private List<Diff> diffMain(
      final String text1, final String text2, final boolean checklines, final long deadline) {
    ...
    if (text1.equals(text2)) {
      ...
      if (text1.length() != 0) diffs.add(new Diff(DiffOperation.EQUAL, text1));
      return diffs;
    }
    ...
    if (commonprefix.length() != 0) diffs.addFirst(new Diff(DiffOperation.EQUAL, commonprefix));
    if (commonsuffix.length() != 0) diffs.addLast(new Diff(DiffOperation.EQUAL, commonsuffix));
  ...
  private List<Diff> diffCompute(
...

    {
      ...
      if (i != -1) {
        ...
        DiffOperation op =
            text1.length() > text2.length() ? DiffOperation.DELETE : DiffOperation.INSERT;
   ...
   * @return LinkedList of Diff objects.
   */
  private List<Diff> diffBisectSplit(
      final String text1, final String text2, final int x, final int y, final long deadline) {
   ...
   * Unicode character represents one line or word .
   *
   ...
   */
  protected LinesOrWordsToCharsResult diffLinesOrWordsToChars(
   ...
   * character represents one line or word.
   *
   ...
   */
  private String diffLinesOrWordsToCharsMunge(
      final String text, final List<String> lineArray, final Map<String, Integer> lineHash) {
    ...
    while (lineOrWordEnd < text.length() - 1) {
      ...
      String separator = "";
      if (diffMode == DiffMode.LINE_LEVEL) lineOrWordEnd = text.indexOf('\n', lineOrWordStart);
      else {
        ...
        } else lineOrWordEnd = -1;
      }
      if (lineOrWordEnd == -1) lineOrWordEnd = text.length();
  ...
  public int diffCommonPrefix(final String text1, final String text2) {
    ...
    for (int i = 0; i < n; i++) if (text1.charAt(i) != text2.charAt(i)) return i;
    return n;
  }
  ...
  public int diffCommonSuffix(final String text1, final String text2) {
    ...
    for (int i = 1; i <= n; i++)
      if (text1.charAt(text1Length - i) != text2.charAt(text2Length - i)) return i - 1;
    return n;
  }
  ...
  protected int diffCommonOverlap(final String text1, final String text2) {
    ...
    if (text1Length > text2Length) resultText1 = resultText1.substring(text1Length - text2Length);
    else if (text1Length < text2Length) resultText2 = resultText2.substring(0, text1Length);
    int textLength = Math.min(text1Length, text2Length);
    // Quick check for the worst case.
    if (resultText1.equals(resultText2)) return textLength;
    ...
    while (true) {
      ...
      length += found;
      if (found == 0
          || resultText1.substring(textLength - length).equals(resultText2.substring(0, length))) {
        best = length;
        length++;
      }
    }
   ...
   *     prefix of text2, the suffix of text2 and the common middle. Or empty string array there was
   *     no match.
   */
  protected String[] diffHalfMatch(final String text1, final String text2) {
    ...
    String[] hm;
    if (hm1.length == 0 && hm2.length == 0) return new String[0];
    else if (hm2.length == 0) hm = hm1;
    else if (hm1.length == 0) hm = hm2;
    else
      // Both matched. Select the longest.
      hm = hm1[4].length() > hm2[4].length() ? hm1 : hm2;

    // A half-match was found, sort out the return data.
    if (text1.length() > text2.length()) return hm;
    // return new String[]{hm[0], hm[1], hm[2], hm[3], hm[4]};
    else return new String[] {hm[2], hm[3], hm[0], hm[1], hm[4]};
  }
   ...
   *     the prefix of shorttext, the suffix of shorttext and the common middle. or empty string
   *     array there was no match.
   */
  ...
  public void diffCleanupSemantic(final List<Diff> diffs) {
    ...
    while (thisDiff != null) {
      if (thisDiff.getOperation() == DiffOperation.EQUAL) {
        ...
        else lengthDeletions2 += thisDiff.getText().length();
        ...
        if (lastequality != null
          ...
          while (thisDiff != equalities.lastElement()) thisDiff = pointer.previous();
          pointer.next();
          ...
          if (equalities.empty())
            ...
            while (pointer.hasPrevious()) pointer.previous();
          else {
            // There is a safe equality we can fall back to.
            thisDiff = equalities.lastElement();

            boolean different = thisDiff != pointer.previous();
            while (different) different = thisDiff != pointer.previous();
          }
  ...
  private void findOverlapsBetweenDeletionAndInsertion(final List<Diff> diffs) {
    ...
    if (pointer.hasNext()) {
      ...
      if (pointer.hasNext()) thisDiff = pointer.next();
    }
    while (thisDiff != null) {
      if (prevDiff.getOperation() == DiffOperation.DELETE
          && thisDiff.getOperation() == DiffOperation.INSERT) {
  ...
  public void diffCleanupSemanticLossless(final List<Diff> diffs) {
    ...
    while (nextDiff != null) {
      if (prevDiff.getOperation() == DiffOperation.EQUAL
          && nextDiff.getOperation() == DiffOperation.EQUAL) {
        ...
        if (!prevDiff.getText().equals(bestEquality1)) {
          ...
          if (bestEquality1.length() != 0) prevDiff.setText(bestEquality1);
          else {
          ...
          if (bestEquality2.length() != 0) nextDiff.setText(bestEquality2);
          else {
  ...
  private int diffCleanupSemanticScore(final String one, final String two) {
    ...
    boolean isWordSeparator1 =
        nonAlphaNumeric1 && (isWordSepartor(Character.toString(char1)) || '.' == char1);
    boolean isWordSeparator =
        nonAlphaNumeric2 && (isWordSepartor(Character.toString(char2)) || '.' == char2);
  ...
  public void diffCleanupEfficiency(final List<Diff> diffs) {
    ...
    while (thisDiff != null) {
      if (thisDiff.getOperation() == DiffOperation.EQUAL) {
        ...
        // An insertion or deletion.
        if (thisDiff.getOperation() == DiffOperation.DELETE) postDel = true;
        else postIns = true;
        /*
        ...
        if (lastequality != null
            && (preIns && preDel && postIns && postDel
                || lastequality.length() < diffEditCost / 2
                    && (preIns ? 1 : 0) + (preDel ? 1 : 0) + (postIns ? 1 : 0) + (postDel ? 1 : 0)
                        == 3)) {
          // System.out.println("Splitting: '" + lastequality + "'");
          // Walk back to offending equality.
          while (thisDiff != equalities.lastElement()) thisDiff = pointer.previous();
      ...
      switch (thisDiff.getOperation()) {
        ...
        case EQUAL:
          if (countDelete + countInsert > 1) {
            ...
            if (bothTypes) {
              ...
              if (commonlength != 0) {
                if (pointer.hasPrevious()) {
                  ...
                  assert thisDiff.getOperation() == DiffOperation.EQUAL
              ...
              if (commonlength != 0) {
                ...
                thisDiff.setText(
                    textInsert.substring(textInsert.length() - commonlength) + thisDiff.getText());
            ...
            if (textDelete.length() != 0) pointer.add(new Diff(DiffOperation.DELETE, textDelete));
            if (textInsert.length() != 0) pointer.add(new Diff(DiffOperation.INSERT, textInsert));
    ...
    while (nextDiff != null) {
      if (prevDiff.getOperation() == DiffOperation.EQUAL
        ...
        if (thisDiff.getText().endsWith(prevDiff.getText())) {
          ...
          thisDiff.setText(
              thisDiff.getText().substring(nextDiff.getText().length()) + nextDiff.getText());
          pointer.remove(); // Delete nextDiff.
          nextDiff = pointer.hasNext() ? pointer.next() : null;
          changes = true;
        }
  ...
  public String diffText1(final List<Diff> diffs) {
    StringBuilder text = new StringBuilder();
    for (Diff aDiff : diffs)
      if (aDiff.getOperation() != DiffOperation.INSERT) text.append(aDiff.getText());
  ...
  public String diffText2(final List<Diff> diffs) {
    StringBuilder text = new StringBuilder();
    for (Diff aDiff : diffs)
      if (aDiff.getOperation() != DiffOperation.DELETE) text.append(aDiff.getText());
    return text.toString();
  }
    ...
    for (Diff aDiff : diffs)
      switch (aDiff.getOperation()) {
        case INSERT:
          try {
            text.append("+")
                .append(URLEncoder.encode(aDiff.getText(), UTF_8).replace('+', ' '))
                .append("\t");
          } catch (UnsupportedEncodingException e) {
            // Not likely on modern system.
    ...
    for (String token : tokens) {
      ...
      switch (token.charAt(0)) {
        ...
        case '=':
          ...
          if (token.charAt(0) == '=') diffs.add(new Diff(DiffOperation.EQUAL, text));
          else diffs.add(new Diff(DiffOperation.DELETE, text));
          break;
        default:
  ...
  protected int matchBitap(final String text, final String pattern, final int loc) {
    ...
    for (int d = 0; d < pattern.length(); d++) {
      ...
      binMid = binMax;
      while (binMin < binMid) {
        if (matchBitapScore(d, loc + binMid, loc, pattern) <= scoreThreshold) binMin = binMid;
        else binMax = binMid;
      ...
      for (int j = finish; j >= start; j--) {
        ...
        if (text.length() <= j - 1 || !s.containsKey(text.charAt(j - 1)))
          // Out of range.
          charMatch = 0;
        else charMatch = s.get(text.charAt(j - 1));
        if (d == 0)
  ...
  protected Map<Character, Integer> matchAlphabet(final String pattern) {
    ...
    for (char c : charPattern) s.put(c, 0);
    int i = 0;
  ...
  protected void patchAddContext(final Patch patch, final String text) {
    ...
    while (text.indexOf(pattern) != text.lastIndexOf(pattern)
      ...
      pattern =
          text.substring(
              Math.max(0, patch.getStart2() - padding),
    ...
    if (prefix.length() != 0) diffs.addFirst(new Diff(DiffOperation.EQUAL, prefix));
    // Add the suffix.
    String suffix =
        text.substring(
            patch.getStart2() + patch.getLength1(),
            Math.min(text.length(), patch.getStart2() + patch.getLength1() + padding));
    if (suffix.length() != 0) diffs.addLast(new Diff(DiffOperation.EQUAL, suffix));
  ...
  public List<Patch> patchMake(final List<Diff> diffs) {
    if (diffs == null) throw new IllegalArgumentException("Null inputs. (patchMake)");
    ...
    for (Diff aDiff : diffs) {
      ...
      switch (aDiff.getOperation()) {
        ...
        case DELETE:
          ...
          break;
        case EQUAL:
          if (aDiff.getText().length() <= 2 * patchMargin
      ...
      if (aDiff.getOperation() != DiffOperation.INSERT) charCount1 += aDiff.getText().length();
      if (aDiff.getOperation() != DiffOperation.DELETE) charCount2 += aDiff.getText().length();
    }
  ...
  public Object[] patchApply(final List<Patch> patches, final String text) {
    ...
    for (Patch aPatch : resultPatches) {
      ...
      } else startLoc = matchMain(resultText, text1, expectedLoc);
      if (startLoc == -1) {
        // No match found. :(
        results[x] = false;
        ...
        if (endLoc == -1)
          text2 =
              resultText.substring(
                  startLoc, Math.min(startLoc + text1.length(), resultText.length()));
        else
          text2 =
              resultText.substring(
                  startLoc, Math.min(endLoc + this.matchMaxBits, resultText.length()));
        if (text1.equals(text2))
          // Perfect match, just shove the replacement resultText in.
          resultText =
              resultText.substring(0, startLoc)
                  + diffText2(aPatch.getDiffs())
            ...
            for (Diff aDiff : aPatch.getDiffs()) {
              if (aDiff.getOperation() != DiffOperation.EQUAL) {
                ...
                if (aDiff.getOperation() == DiffOperation.INSERT)
                  // Insertion
                  resultText =
                      resultText.substring(0, startLoc + index2)
                          + aDiff.getText()
                  ...
                  resultText =
                      resultText.substring(0, startLoc + index2)
                          + resultText.substring(
                              startLoc + diffXIndex(diffs, index1 + aDiff.getText().length()));
              }
              if (aDiff.getOperation() != DiffOperation.DELETE) index1 += aDiff.getText().length();
            }
          }
        }
      }
    ...
    resultText =
        resultText.substring(nullPadding.length(), resultText.length() - nullPadding.length());
  ...
  public String patchAddPadding(final List<Patch> patches) {
    ...
    for (short x = 1; x <= paddingLength; x++) sb.append((char) x);
      ...
      while (!bigPatchDiffs.isEmpty()) {
        ...
        while (!bigPatchDiffs.isEmpty() && patch.getLength1() < patchSize - patchMargin) {
          ...
          if (diffType == DiffOperation.INSERT) {
            ...
            diffText =
                diffText.substring(
                    0, Math.min(diffText.length(), patchSize - patch.getLength1() - patchMargin));
            patch.setLength1(patch.getLength1() + diffText.length());
            start1 += diffText.length();
            ...
            patchDiffs.add(new Diff(diffType, diffText));
            if (diffText.equals(bigPatchDiffs.getFirst().getText())) bigPatchDiffs.removeFirst();
            else
              bigPatchDiffs
                  .getFirst()
                  .setText(bigPatchDiffs.getFirst().getText().substring(diffText.length()));
          }
        }
        ...
        else postcontext = diffText1(bigPatchDiffs);
        if (postcontext.length() != 0) {
          ...
          else patchDiffs.add(new Diff(DiffOperation.EQUAL, postcontext));
        }
        if (!empty) pointer.add(patch);
      }
      bigpatch = pointer.hasNext() ? pointer.next() : null;
    }
  }
  ...
  public String patchToText(final List<Patch> patches) {
    StringBuilder text = new StringBuilder();
    for (Patch aPatch : patches) text.append(aPatch);
    return text.toString();
  }
  ...
  public List<Patch> patchFromText(final String textline) {
    ...
    while (!text.isEmpty()) {
      ...
      if (m.group(2).length() == 0) {
        ...
        patch.setLength1(1);
      } else if (m.group(2).equals("0")) patch.setLength1(0);
      else {
        patch.setStart1(patch.getStart1() - 1);
      ...
      if (m.group(4).length() == 0) {
        ...
        patch.setLength2(1);
      } else if (m.group(4).equals("0")) patch.setLength2(0);
      else {
        patch.setStart2(patch.getStart2() - 1);
