...
public class JavaCompiler {
  ...
  public CompilationUnit compile(
      String unitName, String javaSource, String[] sources, String[] classpaths) {
  ...
  /**
   * Gets the name of the compilation unit that would hypothetically contains the source string to
   * be compiled. Defaults to none (null). The name of the compilation unit must be supplied for
   * resolving bindings. This name should be suffixed by a dot ('.') followed by one of the
   * Java-like extensions and match the name of the main (public) class or interface declared in the
   * source. This name must represent the full path of the unit inside the given project. For
   * example, if the source declares a public class named "Foo" in a project "P" where the source
   * folder is the project itself, the name of the compilation unit must be "/P/Foo.java". If the
   * source declares a public class name "Bar" in a package "p1.p2" in a project "P" in a source
   * folder "src", the name of the compilation unit must be "/P/src/p1/p2/Bar.java".
   *
   * @param context
   * @return
   */
  private String generateUnitName(MergeContext context) {
    ...
    } // in case of any error gives a generic unit name
    return unitName;
  }
  ...
  private String[] findResources(MergeContext context, String fileExtension) {
    ...
    for (String path : projectpaths) {
      if (!path.isEmpty()) {
        List<File> files =
            (List<File>) FileUtils.listFiles(new File(path), new String[] {fileExtension}, true);
        for (File f : files) {
          if (!filespath.contains(f.getParent())) filespath.add(f.getParent());
        }
      }
    }
