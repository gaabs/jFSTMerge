...
public final class TextualMerge {
  ...
  public static String merge(File left, File base, File right, boolean ignoreWhiteSpaces)
    ...
    * String mergeCommand = "";
    ...
    String leftContent =
        ((left == null || !left.exists()) ? "" : FilesManager.readFileContent(left));
    String baseContent =
        ((base == null || !base.exists()) ? "" : FilesManager.readFileContent(base));
    String rightContent =
        ((right == null || !right.exists()) ? "" : FilesManager.readFileContent(right));
  ...
  public static String merge(
      String leftContent, String baseContent, String rightContent, boolean ignoreWhiteSpaces)
      throws TextualMergeException {
    String textualMergeResult = null;
    try {
      RawTextComparator textComparator =
          ((ignoreWhiteSpaces) ? RawTextComparator.WS_IGNORE_ALL : RawTextComparator.DEFAULT);
      ...
      throw new TextualMergeException(
          ExceptionUtils.getCauseMessage(e), leftContent, baseContent, rightContent);
    }
    return textualMergeResult;
  }
}
