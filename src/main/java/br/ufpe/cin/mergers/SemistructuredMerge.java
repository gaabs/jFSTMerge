...
/**
 * Represents semistructured merge. Semistrucutred merge is based on the concept of
 * <i>superimposition</i> of ASTs. Superimposition merges trees recursively, beginning from the
 * root, based on structural and nominal similarities.
 *
 * @author Guilherme
 */
public final class SemistructuredMerge {
  ...
  public static String merge(File left, File base, File right, MergeContext context)
    ...
    try {
      ...
      if (ex
          instanceof
          FileNotFoundException) // FileNotFoundException does not support custom messages
   ...
   */
  private static FSTNode superimpose(
      FSTNode nodeA,
      FSTNode nodeB,
    ...
    if (nodeA.compatibleWith(nodeB)) {
      ...
      if (nodeA instanceof FSTNonTerminal && nodeB instanceof FSTNonTerminal) {
        ...
        for (FSTNode childB : nonterminalB.getChildren()) {
          ...
          if (childA
              == null) { // means that a base node was deleted by left, or that a right node was
                         // added
            FSTNode cloneB = childB.getDeepClone();
            if (childB.index == -1) childB.index = nodeB.index;
            cloneB.index = childB.index;

            nonterminalComposed.addChild(
                cloneB); // cloneB must be removed afterwards if it is a base node
            ...
            if (childA.index == -1) childA.index = nodeA.index;
            if (childB.index == -1) childB.index = nodeB.index;
        ...
        for (int i = 0; i < nonterminalAChildren.size(); i++) {
          ...
          if (childB == null) { // is a new node from left, or a deleted base node in right
            ...
            addNodeToNonTerminalNearNeighbour(
                cloneA, childALeftNeighbour, childARightNeighbour, nonterminalComposed);

            if (context.deletedBaseNodes.contains(
                childA)) { // this is only possible when processing right nodes because this is a
                           // base node not present either in left and right
            ...
            if (isProcessingBaseTree) { // node added by left in relation to base
              ...
              if (!context.addedLeftNodes.contains(childA)) context.nodesDeletedByRight.add(cloneA);
            }
          } else {
            if (!isProcessingBaseTree) {
              context.deletedBaseNodes.remove(
                  childA); // node common to right and base but not to left
            }
          }
        }
        ...
        if (!terminalA.getMergingMechanism().equals("Default")) {
          terminalComposed.setBody(
              markContributions(
                  terminalA.getBody(),
                  terminalB.getBody(),
                  isProcessingBaseTree,
                  terminalA.index,
                  terminalB.index));
        }
  ...
  /**
   * After superimposition, the content of a matched node is the content of those that originated
   * him (left,base,right) So, this methods indicates the origin (left,base or right) in node's body
   * content.
   *
   * @return node's body content marked
   */
  private static String markContributions(
      String bodyA, String bodyB, boolean firstPass, int indexA, int indexB) {
  ...
  /**
   * After superimposition, base nodes supposed to be removed might remain. This method removes
   * these nodes from the merged tree.
   *
   * @param mergedTree
   * @param context
   */
  ...
  /**
   * After superimposition, the content of a matched node is the content of those that originated
   * him (left,base,right). This method merges these parents' content. For instance, calling
   * unstructured merge to merge methods' body. We use the tags from the method {@link
   * #markContributions(String, String, boolean, int, int)} to guide this process.
   *
  ...
  private static void mergeMatchedContent(FSTNode node, MergeContext context)
    ...
    if (node instanceof FSTNonTerminal) {
      ...
      if (((FSTTerminal) node).getBody().contains(SemistructuredMerge.MERGE_SEPARATOR)) {
        ...
        String leftContent =
            splittedBodyContent[0].replace(SemistructuredMerge.SEMANTIC_MERGE_MARKER, "").trim();
        ...
        identifyPossibleNodesDeletionOrRenamings(
            node, context, leftContent, baseContent, rightContent);
      }
    } else {
   ...
   * @param rightContent
   */
  private static void identifyPossibleNodesDeletionOrRenamings(
      FSTNode node,
      MergeContext context,
      String leftContent,
      String baseContent,
  ...
  /**
   * Verifies if a node was edited in only one of the revisions (left, or right), and fills the
   * given merge context with this information.
   *
   * @param node
   * @param context
   * @param leftContent
   ...
   * @param rightContent
   */
  private static void identifyNodesEditedInOnlyOneVersion(
      FSTNode node,
      MergeContext context,
      String leftContent,
      String baseContent,
  ...
  /**
   * Gets the node which comes after a given node (indexed by nodeIndex) in a given list of nodes If
   * the node is the last element in the list, it returns null
   *
   * @param nodes
   * @param nodeIndex
  ...
  /**
   * Inserts a node into a non-terminal node by first trying to find a neighbour which was already
   * inserted so the node can be inserted near such neighbour. In case a neighbour node is not one
   * of non-terminal node's children, it just adds the node at the end of list.
   *
   * @param node
   * @param leftNeighbour
   ...
   * @param nonTerminal
   */
  private static void addNodeToNonTerminalNearNeighbour(
      FSTNode node, FSTNode leftNeighbour, FSTNode rightNeighbour, FSTNonTerminal nonTerminal) {
