...
public final class NewElementReferencingEditedOneHandler {
        ...
        if (isValidNode(addedLeftNode)) {
          for (FSTNode editedRightNode : context.editedRightNodes) {
            if (isValidNode(editedRightNode)) {
              ...
              if (thereIsUnstructuredConflictWithAddedAndEditedElements(
                  unstructuredMergeConflicts, newElementContent, editedElementContent)) {
                if (addedElementRefersToEditedOne(newElementContent, editedElementIdentfier)) {
                  generateConflictWithAddedAndEditedElements(
                      context, ((FSTTerminal) editedRightNode).getBody(), newElementContent);
                }
              }
            }
          }
          ...
          for (FSTNode editedLeftNode : context.editedLeftNodes) {
            if (isValidNode(editedLeftNode)) {
              ...
              if (thereIsUnstructuredConflictWithAddedAndEditedElements(
                ...
                if (addedElementRefersToEditedOne(newElementContent, editedElementIdentfier)) {
                  generateConflictWithAddedAndEditedElements(
                      context, ((FSTTerminal) editedLeftNode).getBody(), newElementContent);
                }
              }
            }
          }
   ...
   */
  private static boolean thereIsUnstructuredConflictWithAddedAndEditedElements(
      List<MergeConflict> unstructuredMergeConflicts, String addedContent, String editedContent) {
  ...
  private static boolean addedElementRefersToEditedOne(
      String newElementContent, String editedElementIdentifier) {
  ...
  private static String getElementIdentifier(FSTNode element) {
    String id =
        (element.getType().equals("MethodDecl"))
            ? element.getName().split("\\(")[0]
            : element.getName();
    return id;
  }

  private static String getEditedElementContent(FSTNode editedelement) {
    // through tests we observed that only the signature (the first line) of an edited method
    // appears in the possible conflict
    String id =
        (editedelement.getType().equals("MethodDecl"))
            ? (((FSTTerminal) editedelement).getBody()).split("\\{")[0]
            : ((FSTTerminal) editedelement).getBody();
    return id;
  }

  /**
   * Creates a merge conflict with the given added and edited elements. It also updates the merged
   * AST with the new merge conflict.
   *
   * @param context
   * @param editedElementContent
   * @param addedElementContent
   */
  private static void generateConflictWithAddedAndEditedElements(
      MergeContext context, String editedElementContent, String addedElementContent) {
    // first creates a conflict with the import statements
    MergeConflict newConflict =
        new MergeConflict(editedElementContent + '\n', addedElementContent + '\n');
    ...
    FilesManager.findAndReplaceASTNodeContent(
        context.superImposedTree, editedElementContent, newConflict.body);
