...
/**
 * Removing inner declarations (high-level elements, FSTNonTerminal nodes) challenges
 * superimposition. For instance, left removed an inner classe edited by right. This handler updates
 * merged tree based on information of the deleted nodes and remaining nodes. The handler combine
 * two solutions. Basically, if the changeset of who edited the original class (without renaming)
 * include a new reference to that class, it keeps both declarations. Otherwise, it joins the
 * content of the two declarations to avoid code duplication. This last solution also verify if the
 * renamed changeset includes a new reference to that class.
 *
 * @author Guilherme
 */
public class DeletionsHandler {
  ...
  private static void manageDeletions(
      MergeContext context, FSTNode deletedNode, FSTNode source, boolean isLeftDeletion) {
    if (deletedNode instanceof FSTNonTerminal) { // high-level elements are FSTNonTerminal nodes
      ...
      if (identifier != null) {
        FSTNode correspondingIdInSource =
            FilesManager.findNodeByID(source, ((FSTTerminal) identifier).getBody());
        if (correspondingIdInSource != null
            && hasChanges(deletedNode, correspondingIdInSource.getParent())) {
          // 2. compare the number of element instances between the base version and the version
          // with the original element (not deleted or renamed)
          // when there are new references to the original element, keep both declarations,
          // otherwise join them
          if (hasNewInstance(context, ((FSTTerminal) identifier).getBody(), isLeftDeletion)) {
            ...
            joinDeclarations(
                context, source, deletedNode, ((FSTTerminal) identifier).getBody(), isLeftDeletion);
          }
  ...
  private static void keepDeclarations(MergeContext context, FSTNode source, String identifier) {
    FSTNode correspondingInMerged =
        FilesManager.findNodeByID(context.superImposedTree, identifier); // TODO add type checking?
  ...
  private static void joinDeclarations(
      MergeContext context,
      FSTNode source,
      FSTNode deletedNode,
      String identifier,
      boolean isLeftDeletion) {
    ...
    if (declarationInMerged != null) {
      ...
      List<FSTNode> candidates =
          (isLeftDeletion) ? context.addedLeftNodes : context.addedRightNodes;
      boolean conflict = true;
      for (FSTNode renamingCandidate : candidates) {
        if (renamingCandidate instanceof FSTNonTerminal) {
          if (hasSameShape(renamingCandidate, deletedNode)
              && hasSimilarContent(renamingCandidate, deletedNode)
              && !hasNewInstance(
                  context, ((FSTTerminal) getId(renamingCandidate)).getBody(), !isLeftDeletion)) {
  ...
  private static boolean hasChanges(FSTNode deletedNode, FSTNonTerminal declarationInSource) {
    return !(hasSameShape(declarationInSource, deletedNode)
        && hasSimilarContent(declarationInSource, deletedNode, 1));
  }
  ...
  private static boolean hasSimilarContent(
      FSTNode candidate, FSTNode deletedNode, double similarityThreshold) {
    String printRenamedCandidate =
        FilesManager.getStringContentIntoSingleLineNoSpacing(
            FilesManager.prettyPrint((FSTNonTerminal) candidate));
    String printDeletedNode =
        FilesManager.getStringContentIntoSingleLineNoSpacing(
            FilesManager.prettyPrint((FSTNonTerminal) deletedNode));
    double similarity =
        FilesManager.computeStringSimilarity(printRenamedCandidate, printDeletedNode);
  ...
  private static void generateNonTerminalConflict(
      FSTNode source,
      String identifier,
      boolean isLeftDeletion,
    ...
    if (correspondingInSource != null) {
      ...
      FSTTerminal terminal =
          new FSTTerminal(declarationInSource.getType(), identifier, newConflict.body, "");
      parent.addChild(terminal, index);
      context.innerDeletionConflicts++;
    }
  }

  private static void joinContent(
      FSTNode source,
      String identifier,
      FSTNonTerminal parent,
      int index,
      FSTNode renamingCandidate) {
  ...
  private static boolean hasNewInstance(
      MergeContext context, String identifier, boolean isLeftDeletion) {
    int baseInstances = countInstances(context.getBase(), identifier);
    int otherInstances =
        countInstances(((isLeftDeletion) ? context.getRight() : context.getLeft()), identifier);
