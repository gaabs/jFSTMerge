...
/**
 * Unstructured merge added false negatives are mostly caused by failing to detect that the
 * contributions to be merged add duplicated declarations. For example, unstructured merge reports
 * no conflict when merging developers contributions that add declarations with the same signature
 * to different areas of the same class. This handler detects such situations. Note that
 * semistructured merge detects such false negatives without handlers, so this handler serves only
...
public final class DuplicatedDeclarationHandler {
  ...
  public static void handle(MergeContext context) {
    ...
    // 3. search and account duplicated declaration errors not surround by conflicts (otherwise, it
    // would be not a false negative)
    ...
    // excluding handler execution time to not bias peformance evaluation as it is not required to
    // original semistructured merge time
