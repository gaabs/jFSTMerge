...
/**
 * Renaming or deletions conflicts happen when one developer edits a element renamed or deleted by
 * other. Semistructured merge is unable to detect such cases because it matches elements via its
 * identifier, so if a element is renamed or deleted it cannot match the elements anymore. This
 * class overcomes this issue.
 *
 * @author Guilherme
 */
public final class RenamingConflictsHandler {
  private static final double BODY_SIMILARITY_THRESHOLD =
      0.7; // a typical value of 0.7 (up to 1.0) is used, increase it for a more accurate
           // comparison, or decrease for a more relaxed one.

  private enum RenamingSide {
    LEFT,
    RIGHT
  }
  ...
  private static void handleMutualRenamings(MergeContext context) {
    ...
    List<FSTNode> leftNewMethodsOrConstructors =
        context
            .addedLeftNodes
            .stream()
            .filter(m -> isMethodOrConstructorNode(m))
    ...
    for (FSTNode left : leftNewMethodsOrConstructors) {
      for (FSTNode right : rightNewMethodsOrConstructors) {
        if (!left.getName()
            .equals(right.getName())) { // only if the two declarations have different signatures
          ...
          if (leftBody.equals(
              rightBody)) { // the methods have the same body, ignoring their signature
            generateMutualRenamingConflict(
                context,
                ((FSTTerminal) left).getBody(),
                ((FSTTerminal) left).getBody(),
                ((FSTTerminal) right).getBody());
          }
          break;
        }
      }
  ...
  private static void handleSingleRenamings(MergeContext context) {
    ...
    handleSingleRenamings(
        context, context.possibleRenamedLeftNodes, context.addedLeftNodes, RenamingSide.LEFT);

    // possible renamings or deletions in right
    handleSingleRenamings(
        context, context.possibleRenamedRightNodes, context.addedRightNodes, RenamingSide.RIGHT);
  }

  private static void handleSingleRenamings(
      MergeContext context,
      List<Pair<String, FSTNode>> possibleRenamedNodes,
      List<FSTNode> addedNodes,
      RenamingSide renamingSide) {
    for (Pair<String, FSTNode> tuple : possibleRenamedNodes) {
      ...
      String currentNodeContent =
          ((FSTTerminal) tuple.getRight()).getBody(); // node content with conflict
      MergeConflict mergeConflict = FilesManager.extractMergeConflicts(currentNodeContent).get(0);
      String oppositeSideNodeContent =
          getMergeConflictContentOfOppositeSide(mergeConflict, renamingSide);
      ...
      List<Pair<Double, String>> similarNodes =
          new ArrayList<>(); // list of possible nodes renaming a previous one
      // 1. getting similar nodes to fulfill renaming conflicts
      for (FSTNode newNode :
          addedNodes) { // a possible renamed node is seem as "new" node due to superimposition
        ...
        double bodySimilarity =
            FilesManager.computeStringSimilarity(baseContent, possibleRenamingContent);
      ...
      boolean hasUnstructuredMergeConflict =
      ...
      if (hasUnstructuredMergeConflict) {
        ...
        generateRenamingConflict(
            context,
            currentNodeContent,
            possibleRenamingContent,
            oppositeSideNodeContent,
            renamingSide);
  ...
  private static String getSignature(String source) {
    ...
    String signatureTrimmed =
        trim.substring(
            0, (/*is interface?*/ (trim.contains("{")) ? trim.indexOf("{") : trim.indexOf(";")));
  ...
  private static void generateRenamingConflict(
      MergeContext context,
      String currentNodeContent,
      String firstContent,
    ...
    // second put the conflict in one of the nodes containing the previous conflict, and deletes the
    // other node containing the possible renamed version
    FilesManager.findAndReplaceASTNodeContent(
        context.superImposedTree, currentNodeContent, newConflict.body);
  ...
  private static void generateMutualRenamingConflict(
      MergeContext context, String currentNodeContent, String firstContent, String secondContent) {
    // statistics
    context.renamingConflicts++;
  ...
  private static String getMergeConflictContentOfOppositeSide(
      MergeConflict mergeConflict, RenamingSide side) {
