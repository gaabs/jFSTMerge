...
/**
 * Semistructured merge uses elements identifier to match nodes, if a node has no identifier, the
 * algorithm is unable to match this node. This lead to problems with initialization block as these
 * elements has no identifier, semistructured merge cannot match them, and therefore creates
 * duplicates. To avoid this problem, we attempt to match them by textual similarity.
 *
 * @author Guilherme
 */
public class InitializationBlocksHandler {
  ...
  public static void handle(
      ...
      List<FSTNode> leftInitlBlocks,
      List<FSTNode> baseInitlBlocks,
      List<FSTNode> rightInitlBlocks)
      throws TextualMergeException {
    List<Triple> matchedInitlBlocks = new ArrayList<Triple>();

    // 1. when there is only a single block, this is the match
    if (leftInitlBlocks.size() == 1
        && rightInitlBlocks.size() == 1
        && baseInitlBlocks.size() == 1) {
      Triple matched =
          new Triple(leftInitlBlocks.get(0), baseInitlBlocks.get(0), rightInitlBlocks.get(0));
      matchedInitlBlocks.add(matched);
    } else {
      // 2. find similar left and right nodes to a base node
      for (FSTNode baseBlock : baseInitlBlocks) {
        // search similar node in left, and remove from the list of left nodes for the next
        // iteration
        FSTNode leftSimilarBlock =
            leftInitlBlocks
                .stream()
        ...
        // search similar node in right, and remove from the list of right nodes for the next
        // iteration
        FSTNode rightSimilarBlock =
            rightInitlBlocks
                .stream()
    ...
    for (Triple tp : matchedInitlBlocks) {
      ...
      if (tp.left != null && tp.right != null) {
        FilesManager.findAndReplaceASTNodeContent(
            context.superImposedTree, leftcontent, mergedContent);
        FilesManager.findAndDeleteASTNode(context.superImposedTree, rightcontent);
      } else if (tp.left == null) {
        FilesManager.findAndReplaceASTNodeContent(
            context.superImposedTree, rightcontent, mergedContent);
