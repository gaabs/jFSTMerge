...
/**
 * Behavioral or compilation type ambiguity errors might occur when import statements share members
 * with the same name (e.g <i>import java.util.List</i> and <i>import java.awt.List</i>, or
 * <i>import java.util.*</i> and <i>import java.awt.*</i>). They also happen when imported members
 * with the same name shared methods with the same name (eg. <i>import java.util.List.add()</i> and
 * <i>import java.awt.List.add()</i>).
 *
 * @author Guilherme
 */
public final class TypeAmbiguityErrorHandler {
  ...
  public static void handle(
      ...
      LinkedList<FSTNode> leftImportStatementsNodes,
      LinkedList<FSTNode> rightImportStatementsNodes) {
    /*
    ...
    if (!leftImportStatementsNodes.isEmpty() && !rightImportStatementsNodes.isEmpty()) {
      List<MergeConflict> unstructuredMergeConflicts =
          FilesManager.extractMergeConflicts(context.unstructuredOutput);
      ...
      while (!leftImportStatementsNodes.isEmpty()) {
        ...
        for (FSTNode rightImportStatementNode : rightImportStatementsNodes) {
          ...
          if ((rightImportedMember.equals("*;") && leftImportedMember.equals("*;"))
            ...
            if (thereIsCompiltationProblemWithImportedStatements(
                compiler, context, leftImportStatement, rightImportStatement)) {
              generateConflictWithImportStatements(
                  context, leftImportStatement, rightImportStatement);
              break;
            }
          ...
          // possible behaviorial type ambiguity error: p.Z vs. q.*
            ...
            if (thereIsUnstructuredConflictWithImportedStatements(
                unstructuredMergeConflicts, leftImportStatement, rightImportStatement)) {
              if (thereIsContributionUsingImportedMember(
                  context, rightImportedMember, leftImportedMember)) {
                generateConflictWithImportStatements(
                    context, leftImportStatement, rightImportStatement);
                break;
              }
  ...
  /**
   * Verifies if the contributions of the class that imported the package refers to the member
   * imported in the other class
   *
   * @param context
   * @param rightImportedMember
   * @param leftImportedMember
   */
  private static boolean thereIsContributionUsingImportedMember(
      MergeContext context, String rightImportedMember, String leftImportedMember) {
    String left =
        (context.getLeft() != null) ? FilesManager.readFileContent(context.getLeft()) : "";
    String base =
        (context.getBase() != null) ? FilesManager.readFileContent(context.getBase()) : "";
    String right =
        (context.getRight() != null) ? FilesManager.readFileContent(context.getRight()) : "";
    if (rightImportedMember.equals("*;")) {
      GoogleTextDiffMatchPatch differ = new GoogleTextDiffMatchPatch();
      List<Diff> differences =
          (!base.equals(""))
              ? differ.diffMainAtLineLevel(base, right)
              : differ.diffMainAtLineLevel(left, right);
      ...
      for (String ctrb : rightContributions) {
        // if(ctrb.contains(membername) && !ctrb.contains("import") &&
        // ctrb.matches("((?s).*\\b\\p{javaJavaIdentifierStart}\\p{javaJavaIdentifierPart}*\\.)*\\p{javaJavaIdentifierStart}\\p{javaJavaIdentifierPart}*\\b.*")){
      ...
      List<Diff> differences =
          (!base.equals(""))
              ? differ.diffMainAtLineLevel(base, left)
              : differ.diffMainAtLineLevel(right, left);
   ...
   */
  private static boolean thereIsCompiltationProblemWithImportedStatements(
      JavaCompiler compiler,
      MergeContext context,
      String leftImportStatement,
      String rightImportStatement) {
    // searching for compilation problems related to type ambiguity error due to the import
    // statements
  ...
  /**
   * Given a list of unstructured merge conflicts, verifies if there is a conflict containing the
   * imported statements.
   *
   ...
   */
  private static boolean thereIsUnstructuredConflictWithImportedStatements(
      List<MergeConflict> unstructuredMergeConflicts,
      String leftImportStatement,
      String rightImportStatement) {
  ...
  /**
   * Creates a merge conflict with the given import statements. It also updates the merged AST with
   * the new merge conflict.
   *
   * @param context
   * @param leftImportStatement
   * @param rightImportStatement
   */
  private static void generateConflictWithImportStatements(
      MergeContext context, String leftImportStatement, String rightImportStatement) {
    // first creates a conflict with the import statements
    MergeConflict newConflict =
        new MergeConflict(leftImportStatement + '\n', rightImportStatement + '\n');
    // second put the conflict in one of the nodes containing the import statements, and deletes the
    // other node containing the orther import statement
    FilesManager.findAndReplaceASTNodeContent(
        context.superImposedTree, leftImportStatement, newConflict.body);
