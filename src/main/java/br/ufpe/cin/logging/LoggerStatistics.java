...
public class LoggerStatistics {
  ...
  static {
    if (!JFSTMerge.isCryptographed) {
      try {
        String logpath =
            System.getProperty("user.home") + File.separator + ".jfstmerge" + File.separator;
  ...
  public static void logScenario(String loggermsg) throws IOException {
    ...
    String header =
        "revision,ssmergeconfs,ssmergeloc,ssmergerenamingconfs,ssmergedeletionconfs,ssmergeinnerdeletionconfs,ssmergetaeconfs,ssmergenereoconfs,"
  ...
  public static void logConflicts(List<MergeConflict> conflicts, Source source) throws IOException {
    ...
    for (MergeConflict mc : conflicts) {
      String origin =
          ((mc.leftOriginFile != null) ? mc.leftOriginFile.getAbsolutePath() : "<empty left>")
              + ";"
              + ((mc.baseOriginFile != null) ? mc.baseOriginFile.getAbsolutePath() : "<empty base>")
              + ";"
              + ((mc.rightOriginFile != null)
                  ? mc.rightOriginFile.getAbsolutePath()
                  : "<empty right>");
      if (source == null) {
  ...
  private static void logSummary() throws IOException {
    try {
      ...
      if (statistics.exists()) {
        ...
        // int FP_UN = (unmergeconfs - ssmergeconfs) + duplicateddeclarationerrors -
        // (ssmergetaeconfs + ssmergenereoconfs + ssmergeinitlblocksconfs);FP_UN=(FP_UN>0)?FP_UN:0;
        int FP_UN = unmergeorderingconfs;
        int FN_UN = duplicateddeclarationerrors;
        int FP_SS =
            ssmergerenamingconfs; // actually they are common renaming conflicts, not additional
                                  // false positives
        int FN_SS =
            (ssmergetaeconfs + ssmergenereoconfs + ssmergeinitlblocksconfs) + ssmergeacidentalconfs;
  ...
  private static void logFiles(String timeStamp, MergeContext context) throws IOException {
    try {
      ...
      if (!leftcontent.isEmpty()) {
        ...
        FileUtils.write(
            logfiles, timeStamp + "," + context.getLeft().getAbsolutePath() + "\n", true);
      ...
      if (!basecontent.isEmpty()) {
        ...
        FileUtils.write(
            logfiles, timeStamp + "," + context.getBase().getAbsolutePath() + "\n", true);
      ...
      if (!rightcontent.isEmpty()) {
        ...
        FileUtils.write(
            logfiles, timeStamp + "," + context.getRight().getAbsolutePath() + "\n", true);
  ...
  private static void initializeLogger() throws IOException, CryptoException {
    ...
    String header =
        "date,files,ssmergeconfs,ssmergeloc,ssmergerenamingconfs,ssmergedeletionconfs,ssmergeinnerdeletionconfs,ssmergetaeconfs,ssmergenereoconfs,"
            + "ssmergeinitlblocksconfs,ssmergeacidentalconfs,unmergeconfs,unmergeloc,unmergetime,ssmergetime,unmergeduplicateddeclarationerrors,"
            + "unmergeorderingconfs,equalconfs\n";
  ...
  private static StringBuilder fillSummaryMsg(
      int ssmergeconfs,
      int ssmergeloc,
      int unmergeconfs,
      int unmergeloc,
    ...
    if (FP_UN > 0 && FN_UN > 0) {
      ...
      summary.append(
          " and at least " + FN_UN + " false negative(s) in relation to unstructured merge.\n");
    } else if (FP_UN == 0 && FN_UN == 0) {
      summary.append(
          "In these files, S3M did not find any occurrence of unstructured merge false positives and false negatives.\n");
    } else if (FP_UN > 0 && FN_UN == 0) {
      summary.append(
          "In these files, you avoided at least "
              + FP_UN
              + " false positive(s), and S3M did not find any occurrence of unstructured merge false negatives.\n");
    } else if (FP_UN == 0 && FN_UN > 0) {
      summary.append(
          "In these files, S3M did not find any occurrence of unstructured merge false positives, but you avoided at least "
              + FN_UN
              + " false negative(s) in relation to unstructured merge.\n");
    }
    ...
    if (FN_SS == 0) {
      ...
      summary.append(
          " you had no extra false positives, but you had at most "
              + FN_SS
              + " potential extra false negative(s).");
    }
    ...
    summary.append(
        " compared to "
            ...
            + " conflicts and "
            + unmergeloc
            + " conflicting LOC from unstructured merge.");
    /*		if(equalconfs >0){
    ...
    if (ssmergeconfs != unmergeconfs) {
      if (ssmergeconfs < unmergeconfs) {
        summary.append(
            "these numbers represent a reduction of "
                + String.format(
                    "%.2f",
                    ((double) ((unmergeconfs - ssmergeconfs) / (double) unmergeconfs)) * 100)
                + "% in the number of conflicts by S3M.\n");
      } else if (ssmergeconfs > unmergeconfs) {
        summary.append("these numbers represent no reduction of conflicts by S3M.\n");
      }
    } else {
      summary.append(
          "these numbers represent no difference in terms of number of reported conflicts.\n");
    }

    if (FP_UN > 0) {
      summary.append(
          "A reduction of "
              + String.format("%.2f", ((double) ((FP_UN - 0) / (double) FP_UN)) * 100, 2)
    ...
    if (FN_UN != FN_SS) {
      if (FN_UN > FN_SS) {
        summary.append(
            "And a reduction of "
                + String.format("%.2f", ((double) ((FN_UN - FN_SS) / (double) FN_UN)) * 100, 2)
    ...
    summary.append(
        "\n\nFinally, S3M took "
            + (new DecimalFormat("#.##").format(M))
            + " seconds, and unstructured merge "
            + (new DecimalFormat("#.##").format(N))
            + " seconds to merge all these files.");
