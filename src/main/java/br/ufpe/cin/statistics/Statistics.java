...
public final class Statistics {
  ...
  /**
   * Computes number of conflicts, and conflicting lines of code of the merged files in a given
   * merge context. The result is logged in the <i>jfstmerge.log</i> file.
   *
   * @param context
   * @throws Exception
   */
  public static void compute(MergeContext context) throws Exception {
    List<MergeConflict> semistructuredMergeConflicts =
        FilesManager.extractMergeConflicts(context.semistructuredOutput);
    List<MergeConflict> unstructuredMergeConflits =
        FilesManager.extractMergeConflicts(context.unstructuredOutput);

    context.semistructuredNumberOfConflicts =
        computeNumberOfConflicts(semistructuredMergeConflicts);
    ...
    context.equalConflicts =
        computeEqualConflicts(unstructuredMergeConflits, semistructuredMergeConflicts);

    context.orderingConflicts =
        (context.unstructuredNumberOfConflicts - context.semistructuredNumberOfConflicts)
    ...
    context.acidentalConflicts =
        context.unstructuredNumberOfConflicts - context.equalConflicts - context.orderingConflicts;
    ...
    String filesMerged =
        ((context.getLeft() != null) ? context.getLeft().getAbsolutePath() : "<empty left>")
            ...
            + "#"
            + ((context.getRight() != null)
                ? context.getRight().getAbsolutePath()
                : "<empty right>");
    String loggermsg =
        filesMerged
            + ","
  ...
  private static int computeConflictsLOC(List<MergeConflict> listofconflicts) {
    ...
    for (MergeConflict mc : listofconflicts) {
      conflictsloc +=
          ((new BufferedReader(new StringReader(mc.left))
                  .lines()
                  .collect(Collectors.toList())
                  .size())
              + (new BufferedReader(new StringReader(mc.right))
                  .lines()
                  .collect(Collectors.toList())
                  .size()));
    }
  ...
  /**
   * Calculates textually equal conflicts from given list of unstructured and semistructured merge
   * conflicts.
   *
   ...
   */
  private static int computeEqualConflicts(
      List<MergeConflict> unstructuredMergeConflits,
      List<MergeConflict> semistructuredMergeConflicts) {
    int equalconfs = 0;
  ...
  /**
   * Computes and print textually equal and different conflicts from a given merge context/merged
   * file.
   *
   * @param scenario
   * @throws IOException
