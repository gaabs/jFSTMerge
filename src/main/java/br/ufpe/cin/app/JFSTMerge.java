    ...
    public static void main(String[] args) {
        try {
            ...
            for (String revison : listRevisions) {
                ...
                boolean hasRenamingConflict = mergeScenario.getTuples().stream()
                        .anyMatch(x -> x.getContext().renamingConflicts > 0);

                if (hasRenamingConflict) {
                    System.out.println("Found renaming");
                    renamings.add(revison);
                }
            }

            BufferedWriter writer = Files.newBufferedWriter(Paths.get("renamings.revisions"));
            renamings.forEach(revison -> {
                try {
                    writer.write(revison + "\n");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            });
