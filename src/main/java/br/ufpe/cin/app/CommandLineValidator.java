...
public final class CommandLineValidator {
  ...
  public static void validateCommandLineOptions(JFSTMerge merger) {
    if (merger.filespath.isEmpty()
        && merger.directoriespath.isEmpty()) { // one merge option must be given
      throw new ParameterException("Please inform one merge option (-f or -d).");

    } else if (!merger.filespath.isEmpty()
        && !merger.directoriespath.isEmpty()) { // merge options are mutually exclusive
      throw new ParameterException(
          "Choose only one of the merge options (files -f or directories -d), not both");
